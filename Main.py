...
def imagens():
    ...
    for r in regs:
        ...
        usuario_foto_url = None
        if r.usuario_foto:
            if isinstance(r.usuario_foto, str) and r.usuario_foto.startswith('http'):
                usuario_foto_url = r.usuario_foto
            else:
                usuario_foto_url = f'/uploads/{os.path.basename(r.usuario_foto)}'
        result.append({
            'id': r.id,
